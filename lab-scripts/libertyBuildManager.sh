...
#Add the plantsbywebsphere application to the server,  
echo "" | tee -a $LOG
...
#Add the WhereAmI applications to the server, which is used in the Dynamic Routing scenario.
  echo "" | tee -a $LOG
  ...
  cp $LAB_FILES_DIR/WhereAmI.war $WLP_HOME/usr/servers/$PBW_SERVER_NAME/dropins/.
  
  
#Create the Liberty shared config directory
echo "" | tee -a $LOG
...
#Add the DB2 libraries to the Liberty shared config
echo "" | tee -a $LOG
...
#Update the server.xml file based on TA generated file
echo "" | tee -a $LOG
